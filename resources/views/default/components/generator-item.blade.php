...
>
    <x-lqd-icon
        class="mb-5 lqd-generator-item-icon group-hover:scale-110 group-hover:shadow-lg"
        size="xl"
        style="background: {{ $item->color }}"
        active-badge
        active-badge-condition="{{ $item->active == 1 }}"
    >
        <span class="flex items-center justify-center transition-transform size-5 group-hover:scale-110">
            @if ($item->image !== 'none')
