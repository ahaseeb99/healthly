...
<div {{ $attributes->withoutTwMergeClasses()->twMerge($base_class, $attributes->get('class')) }}>
    <div>
        <div class="flex h-2 overflow-hidden rounded-full lqd-progress">
            @if ($total_documents != 0)
                <div
                    class="lqd-progress-bar shrink-0 grow-0 basis-auto prgress-bar-primary"
                    style="width: {{ (100 * (int) $total_text_documents) / (int) $total_documents }}%"
                ></div>
            @endif
            @if ($setting->feature_ai_image && $total_documents != 0)
                <div
                    class="lqd-progress-bar shrink-0 grow-0 basis-auto prgress-bar-secondary"
