...
<header class="lqd-header relative flex h-[--header-height] border-b text-xs font-medium transition-colors max-lg:h-[65px]">
    <div @class([
        ...
        <div class="flex items-center gap-3 mobile-nav-logo lg:hidden">
            <button
                class="flex items-center justify-center lqd-mobile-nav-toggle size-10"
                type="button"
                x-init
        ...
        {{-- Title slot --}}
        @if ($title ?? false)
            <div class="items-center hidden header-title-container peer/title lg:flex">
        ...
        <div class="flex justify-end gap-4 header-actions-container grow max-lg:basis-2/3 max-lg:gap-2">
                ...
                <div class="flex items-center max-xl:gap-2 max-lg:hidden xl:gap-3">
                    ...
                    @if ($settings_two->liquid_license_type == 'Extended License')
                        ...
                        <x-button
                            ...
                            href="{{ route('dashboard.user.payment.subscription') }}"
                        >
                            <x-tabler-bolt class="fill-current size-4" />
            ...
            <div class="flex items-center gap-4 max-lg:gap-2">
                        ...
                        >
                            @foreach (LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                                @if (in_array($localeCode, explode(',', $settings_two->languages)))
                                    <a
                                        class="flex items-center gap-2 px-3 py-2 transition-colors border-b text-heading-foreground last:border-b-0 hover:bg-foreground/5 hover:no-underline"
                                        rel="alternate"
                                        hreflang="{{ $localeCode }}"
                ...
                >
                    ...
                    >
                        <span
                            class="inline-block bg-cover rounded-full size-full"
                    ...
                    >
                        ...
                        <div class="pb-2 text-2xs">
                            <a
                                class="flex items-center w-full px-3 py-2 hover:bg-foreground/5"
                                href="{{ route('dashboard.user.payment.subscription') }}"
                            >
                                {{ __('Plan') }}
                            </a>
                            ...
                            >
                                @csrf
                                <button
                                    class="flex items-center w-full px-3 py-2 hover:bg-foreground/10"
                                    type="submit"
                                >
                                    {{ __('Logout') }}
                                </button>
