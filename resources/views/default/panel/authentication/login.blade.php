...
@section('form')
    <h1 class="mb-11 sign-text">{{ __('Sign in') }}</h1>
    ...
    <form class="flex flex-col gap-6" novalidate="novalidate" onsubmit="return LoginForm();">
        ...
        <div class="flex justify-between gap-2 my-2">
            ...
            <div class="ms-auto text-end">
                <a class="text-indigo-600" href="{{ route('forgot_password') }}" style="color: white;">{{ __('Forgot Password?') }}</a>
            </div>
        </div>
        ...
        @if ($setting->privacy_enable == 1 && $setting->privacy_enable_login == 1)
            <div class="text-foreground/70 sign-text">
                {{ __('By proceeding, you acknowledge and accept our') }}
                <a class="font-medium underline text-foreground sign-text" href="{{ url('/terms') }}" target="_blank">
                    {{ __('Terms and Conditions') }}
                </a>
                {{ __('and') }}
                <a class="font-medium underline text-foreground sign-textw sign-text" href="{{ url('/privacy-policy') }}" target="_blank">
                    {{ __('Privacy Policy') }}
                </a>.
            </div>
        @endif
    ...
    @if ($setting->register_active == 1)
        <div class="mt-20">
            ...
            <a class="font-medium text-indigo-600 underline" href="{{ route('register', ['plan' => $plan]) }}" style="color: white;">
                {{ __('Sign up') }}
            </a>
        </div>
    @endif
                    <p style="margin-top: 50px">GlobalAIVectra s.r.o. - Vovks Mareks - 371 25925475 - Kurta Konráda 2517/1, Libeň, 190 00 Praha 9 | hello@healthly.live</p>
