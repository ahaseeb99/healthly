...
<head>
    ...
    <style>
        ...
        :is(.theme-dark .dark\:before\:bg-primary):before {
            background-color: #ffdc00;
        }
        .group\/link:hover .group-hover\/link\:text-navbar-icon-foreground-hover {
            color: #ffdc00;
        }
        .lqd-navbar-label,
        .lqd-legend-label {
        ...
        .prgress-bar-primary {
            background-color: #7cfc01;
        }
        .prgress-bar-secondary {
     ...
     @auth
               <body class="text-xs antialiased group/body bg-background font-body text-foreground transition-bg" data-email="{{ auth()->user()->email }}" data-name="{{ auth()->user()->name }}" >
