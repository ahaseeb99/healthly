...
<!-- <header @class([
    'site-header group/header absolute inset-x-0 z-50 text-white transition-[background,shadow] [&.lqd-is-sticky]:bg-white [&.lqd-is-sticky]:text-black [&.lqd-is-sticky]:shadow-[0_4px_20px_rgba(0,0,0,0.03)]',
    'top-12' => $fSectSettings->preheader_active,
    'top-1' => !$fSectSettings->preheader_active,
])>
    <nav
        class="relative flex items-center justify-between border-opacity-10 px-7 py-4 text-[14px] opacity-0 transition-all duration-500 group-[.lqd-is-sticky]/header:border-black group-[.lqd-is-sticky]/header:border-opacity-5 group-[.page-loaded]/body:opacity-100 max-sm:px-2"
        id="frontend-local-navbar" style="font-size:12px"
    >
        <a
...
@includeWhen(
    in_array($settings_two->chatbot_status, ['frontend', 'both']) 
