...
class AIController extends Controller
{
    ...
    public function buildOutput(Request $request)
    {
		...
		if ($post->type == 'rss') {
			...
			$prompt .= "Your result must be in $language language. Number of results should be $number_of_results. And the maximum length of $maximum_length characters. Tone of voice must be $tone_of_voice";
		}
	...
	private function pdfStream(Request $request)
	{
		...
		return self::openaiChatStream($openaiApiKey, $chat_bot, $history);
	}
	...
	private function visionStream(Request $request)
	{
		...
		return self::openaiChatStream($openaiApiKey, $chat_bot, $history, 2000);
	}
	private function openaiChatStream($openaiApiKey, $chat_bot, $history, $ai_max_tokens = null){
		$gclient = new Client();
		...
		if ($ai_max_tokens !== null) {
			$postData['json']['max_tokens'] = $ai_max_tokens;
		}
		return response()->stream(function () use ($url, $gclient, $postData) {
