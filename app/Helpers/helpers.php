...
function getSubscriptionName()
{
    ...
    // Assuming getSubscription() returns the subscription object or null
    $subscription = getSubscription();

    if ($subscription) {
        // Retrieve the payment plan
        $paymentPlan = PaymentPlans::where('id', $subscription->name)->first();

        // If payment plan is found, return its name
        if ($paymentPlan) {
            return $paymentPlan->name;
        }
    }

    // Return "Custom subscription" if subscription or payment plan is not found
